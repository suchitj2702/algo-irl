  ...
  javascript: {
    ...
    generateWrapperCode: (code, testCases) => {
      ...
      return `
        ...
        // Helper for safely comparing arrays regardless of order
        function arraysEqual(arr1, arr2) {
          ...
          // For arrays with objects, do a deep comparison
        ...
        // Helper for safely serializing values
        function safeSerialize(value) {
          ...
          // Handle special non-serializable types
          ...
          if (Array.isArray(value)) {
            try {
              ...
              return value.map(() => '[Complex value]');
            }
          }
        ...
        for (let i = 0; i < testCases.length; i++) {
          try {
            ...
            // Make sure the result is safe for transmission
            const safeResult = safeSerialize(result);
            
            // Compare result with expected output
            let passed = false;
            
            // For arrays, use the custom comparison function
            if (Array.isArray(expected) && Array.isArray(result)) {
              passed = arraysEqual(result, expected);
            } 
            // Handle special cases where expected is a primitive but result might be an object
            else if (typeof expected !== 'object' && typeof result === 'object' && result !== null) {
              try {
            ...
            // For other types, compare directly
              ...
              try {
                if (typeof expected === 'object' && expected !== null) {
                  const expectedStr = JSON.stringify(expected);
                  const resultStr = JSON.stringify(safeResult);
                  passed = expectedStr === resultStr;
                } else {
                  passed = safeResult === expected;
                }
              } catch (e) {
                passed = false;
              }
            ...
            // Store test result with safe output - convert complex objects to strings
            let outputForStorage;
            try {
              // Convert complex objects to strings to avoid transfer issues
              if (safeResult === null || 
                  safeResult === undefined || 
                  typeof safeResult === 'string' || 
                  typeof safeResult === 'number' || 
                  typeof safeResult === 'boolean') {
                outputForStorage = safeResult;
              } else {
                outputForStorage = JSON.stringify(safeResult);
              }
            } catch (e) {
              outputForStorage = '[Non-serializable result]';
            }
            
            results.testResults.push({
              testCase: testCase,
              passed: passed,
              actualOutput: outputForStorage
            });
            ...
            console.error('Test case error:', error.message || String(error));
          }
        }
        console.timeEnd('execution');
        
        // Expose results to isolated-vm
        results;
      `;
    }
  },
...
print(json.dumps(results))
  ...
  java: {
    fileExtension: '.java',
    prepareCommand: (filename = '') => `javac ${filename}.java && java ${filename}`,
    validateCode: (code) => {
      // Check for potentially dangerous operations
      const dangerousPatterns = [
        /System\.exit/i,
...
public class ${className} {
  ...
  csharp: {
    fileExtension: '.cs',
    prepareCommand: (filename = '') => `dotnet script ${filename}.cs`,
    validateCode: (code) => {
      // Check for potentially dangerous operations
      const dangerousPatterns = [
...
class Program
{
  ...
  cpp: {
    fileExtension: '.cpp',
    prepareCommand: (filename = '') => `g++ -std=c++17 ${filename}.cpp -o ${filename} && ./${filename}`,
    validateCode: (code) => {
      // Check for potentially dangerous operations
      const dangerousPatterns = [
        /system\s*\(/i,
...
int main() {
  ...
  go: {
    fileExtension: '.go',
    prepareCommand: (filename = '') => `go run ${filename}.go`,
    validateCode: (code) => {
      // Check for potentially dangerous operations
      const dangerousPatterns = [
        /os\./i,
	...
	// Test case ${i + 1}
	{
		...
		func() {
			...
			${Object.entries(tc.input).some(([key]) => key === 'target') ? 
				`result = solution(
					${Object.entries(tc.input).find(([key]) => key === 'nums')?.[1].map((n) => n).join(', ')},
					${Object.entries(tc.input).find(([key]) => key === 'target')?.[1]}
				)` : 
				`result = solution(${Object.entries(tc.input).find(([key]) => key === 'nums')?.[1].map((n) => n).join(', ')})`
			}
		}()

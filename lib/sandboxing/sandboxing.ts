// Sandboxing functionality for code execution using Node.js VM
import vm from 'vm';
...
/**
 * Executes code in Node.js built-in VM
 * Provides a secure execution environment
 */
export async function executeInNodeVm(code: string, options: SandboxOptions = {}): Promise<SandboxResult> {
  ...
  const sandbox: Record<string, any> = {
    console: {
      ...
      time: (label: string) => console.time(String(label)),
      timeEnd: (label: string) => console.timeEnd(String(label))
    },
    setTimeout: () => {}, // No-op
    setInterval: () => {}, // No-op
    clearTimeout: () => {}, // No-op
  ...
  sandbox.__printResult = (data: any) => {

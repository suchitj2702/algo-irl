...
export class AnthropicService {
  ...
  /**
   * Transforms a coding problem into a company-specific interview scenario
   */
  async transformToCompanyScenario({ 
    problem, 
    company, 
    useCache = true 
  }: TransformOptions): Promise<string> {
    // Generate a cache key based on problem and company
    const cacheKey = `${problem.title}-${company}`.toLowerCase().replace(/\s+/g, '-');
    ...
    const prompt = this.buildPrompt(problem, company);
    ...
    while (retries <= MAX_RETRIES) {
      try {
        const response = await this.client.messages.create({
          ...
          system: "You are an AI that transforms coding problems into realistic company-specific interview scenarios that test the same underlying algorithm or data structure concepts. Your responses should be concise, clear, and directly applicable to a technical interview setting."
        });
...
Your scenarios should feel like actual questions a candidate would receive in a technical interview, with appropriate domain-specific framing that aligns with the company's business and technology.`
   ...
   */
  private buildPrompt(problem: TransformOptions['problem'], company: string): string {
    return `
For this coding problem, create an alternative way of presenting the same problem with ${company}-specific context. 
...
Transform this into a realistic interview scenario that a ${company} interviewer might present during a technical interview. The scenario should:
1. Maintain the exact same algorithmic approach and complexity as the original
2. Use the same input/output structure
3. Include clear examples with test cases similar to the original problem
4. Add a context or frame story related to ${company}'s products or services
5. Keep the same time and space complexity requirements 
6. Be concise and clear

IMPORTANT: Do not fundamentally change the problem - only the context and story. The original problem's algorithm, data structures, input types, and complexity requirements must remain identical. Ensure test cases are included that match the original problem structure.
`;
  }

import { Company, Problem, ProblemDifficulty } from '@/types/entities';
import { getCompanyById, getAllCompanies } from './company';
import { getProblemById } from './firestoreUtils';
...
/**
 * Main function to create an enhanced prompt context
 */
export async function createEnhancedPromptContext(
  problemId: string, 
  companyId: string
): Promise<EnhancedPromptContext | null> {
  try {
    ...
    console.error('Error creating enhanced prompt context:', error);
    return null;
  }
}
...
/**
 * Generate an enhanced prompt for the Anthropic API
 */
export function generateEnhancedPrompt(context: EnhancedPromptContext): string {
